...
describe('ddb-utils', () => {
	describe('#duplicateObject()', () => {
		it('should support null, string, int, Date', () => {
			let source = null;
			...
			assert.strictEqual(DDB.duplicateObject(source).toString("utf-8"), source.toString("utf-8"));
			source = () => null;
			assert.throws(() => DDB.duplicateObject(source), /Unsupported value type/);
		});
		it('should support arrays', () => {
			...
			assert.deepStrictEqual(DDB.duplicateObject(source), source);
			source = [1, "2", false];
			const copy = DDB.duplicateObject(source);
