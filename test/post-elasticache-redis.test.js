...
describe('post-elasticache-redis', () => {
	let processRequest = null;
	before(() => {
		sinon.stub(redis, 'servers', () => {
			return Promise.resolve({});
		});
		sinon.stub(redis, 'set', (slots, records, options) => {
			try {
	...
	it('store item', () => {
		const key     = uuid();
		const target  = { destination: `localhost:6379` };
