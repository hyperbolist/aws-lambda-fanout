...
const post         = require('../lib/post-firehose.js');

describe('post-firehose', () => {
	let processRequest = null;
	before(() => {
		AWS.mock('Firehose','putRecordBatch', (params) => {
	...
	it('configure', (done) => {
		...
		assert.strictEqual(conf.service, 'Firehose');
	...
	it('store item (with params)', () => {
		...
		processRequest = (params) => {
			assert.deepEqual(params.Records, [ { Data: new Buffer(`AZERTY${key}`, `utf-8`) } ]);
		};
	...
	it('store items', () => {
		...
		processRequest = (params) => {
			assert.deepEqual(params.Records, [ { Data: new Buffer(`AZERTY${key}`, `utf-8`) }, { Data: new Buffer(`QWERTY${key}`, `utf-8`) } ]);
		};
