...
function failure(message, err, record, key) {
	return { success: [], errors: [ { message: message, error: err, record: record, key: key } ] };
}
...
function transformDDBRecord(record, convertDDB) {
	try {
		...
		const key = record.dynamodb ? JSON.stringify(record.dynamodb.Keys) : "undefined";
		return Promise.resolve(failure("Unable to deserialize DynamoDB record, removing it", e, record, key));
	}
}
...
function transformKinesisSingleRecord(record) {
	try {
		...
		const key = record.kinesis ? `${record.kinesis.partitionKey}#${record.kinesis.sequenceNumber}` : "undefined";
		return Promise.resolve(failure("Unable to deserialize Kinesis record, removing it", e, record, key));
	}
}
...
function transformKinesisAggregatedRecords(record) {
	return new Promise((resolve, reject) => {
		try {
			...
			const key = record.kinesis ? `${record.kinesis.partitionKey}#${record.kinesis.sequenceNumber}` : "undefined";
			resolve(failure("Unable to deserialize KPL record, removing it", e, record, key));
		}
	});
}
...
function transformSNSRecord(record) {
	try {
		...
		const key = record.Sns ? record.Sns.MessageId : "undefined";
		return Promise.resolve(failure("Unable to deserialize SNS record, removing it", e, record, key));
	}
}
...
function transformFirehoseRecord(record) {
	try {
		...
		const key = record.firehose ? record.firehose.recordId : "undefined";
		return Promise.resolve(failure("Unable to deserialize Firehose record, removing it", e, record, key));
	}
}
