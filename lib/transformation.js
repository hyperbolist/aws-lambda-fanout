...
function transformDDBRecord(record, target, callback) {
  try {
    ...
    if (target.jsonifyFields) {
      object = jsonifyFields(object, target.jsonifyFields);
    }
    if (target.promoteFields) {
      object = promoteFields(object, target.promoteFields);
    }
...
//********
// This function parses stringified fields as json
function jsonifyFields(object, fieldNames) {
  var jsonified = JSON.parse(JSON.stringify(object));
  fieldNames.split(',').forEach(function(fieldName) {
    // if the field exists on the object
    if (jsonified[fieldName]) {
      // try to parse it as json
      try {
        jsonified[fieldName] = JSON.parse(jsonified[fieldName]);
      }
      catch(e) {
        console.warn("Unable to parse " + fieldName + " as JSON, leaving it as-is: " + jsonified[fieldName], e.stack);
      }
    }
  });
  return jsonified;
...
//********
// This function promotes json fields' children
function promoteFields(object, fieldNames) {
  var promoted = JSON.parse(JSON.stringify(object));
  fieldNames.split(',').forEach(function(fieldName) {
    // if the field exists on the object
    if (promoted[fieldName]) {
      // try to promote its children and remove it
      try {
        var field = promoted[fieldName];
        Object.keys(field).forEach(function(childName) {
          promoted[childName] = field[childName];
        });
        delete promoted[fieldName];
      }
      catch(e) {
        console.warn("Unable to promote " + fieldName + "'s children', leaving it as-is", e.stack);
      }
    }
  });
  return promoted;
