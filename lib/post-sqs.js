...
exports.destinationRegex = /^https:\/\/((queue)|(sqs\.[a-z]+-[a-z]+-[0-9])|([a-z]+-[a-z]+-[0-9]\.queue))\.amazonaws\.com\/[0-9]{12}\/[a-zA-Z0-9_-]{1,80}$/;
...
exports.create = function(target, options) {
	...
	const service = {
		instance: new AWS.SQS(serviceOptions),
		debug: config.debug
	};
...
exports.send = function(service, target, records) {
	switch(target.collapse) {
		case "JSON": {
			...
			const entries = { Records: records.map((record) => JSON.parse(record.data.toString('utf-8'))) };
			return service.instance.sendMessage({ QueueUrl: target.destination, MessageBody: JSON.stringify(entries) }).promise();
		}
		case "concat-b64": {
			// We have multiple messages, collapse them in a single buffer
			const data = Buffer.concat(records.map((record) => record.data));
			return service.instance.sendMessage({ QueueUrl: target.destination, MessageBody: data.toString('base64') }).promise();
		}
		case "concat": {
			let data = null;
			// We have multiple messages, collapse them in a single buffer
			if(target.separator && target.separator.length > 0) {
				data = records.map((record) => record.data.toString('utf-8')).join(target.separator);
			} else {
				data = Buffer.concat(records.map((record) => record.data)).toString('utf-8');
			}
			return service.instance.sendMessage({ QueueUrl: target.destination, MessageBody: data }).promise();
		}
		default: {
			if(records.length != 1) {
				return Promise.reject(new Error(`Multiple records must be collapsed, SQS supports JSON|concat-b64|concat`));
			}
			// We have a single message, let's send it
			return service.instance.sendMessage({ QueueUrl: target.destination, MessageBody: records[0].data.toString('utf-8') }).promise();
		}
	}
};
