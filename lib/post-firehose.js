...
module.exports.create = function(target, options) {
	...
	const service = {
		instance: new AWS.Firehose(serviceOptions),
		debug: config.debug
	};
...
module.exports.send = function(service, target, records) {
	const entries = records.map((record) => { return { Data: record.data }; });
	return service.instance.putRecordBatch({ DeliveryStreamName: target.destination, Records: entries }).promise();
};
