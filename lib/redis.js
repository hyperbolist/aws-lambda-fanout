...
var defaultValues = {
  ...
  tunnel    : configuration.getEnvironmentVariable('REDIS_TUNNEL_MODE', "false") == "true",
  tunnelHost: configuration.getEnvironmentVariable('REDIS_TUNNEL_HOST', ""),
  tunnelPort: Number(configuration.getEnvironmentVariable('REDIS_TUNNEL_PORT', 0)),
  timeout   : configuration.getEnvironmentVariable('REDIS_CLIENT_TIMEOUT', configuration.getEnvironmentVariable('CLIENT_TIMEOUT', 0))
};
...
exports.set = function(server, records, callback) {
  ...
  try {
    ...
    return Promise.reject(new Error("Error occured while sending item to Redis: " + e));
  }

  return new Promise((resolve, reject) => {
    let response = "";
    // Send first command to the server
    const client = net.connect({ host: host, port: port }, function() {
    ...
    client.on('data', function(chunk) {
      ...
      if(index != -1) {
        ...
        if(result != "OK") {
          ...
          reject(new Error("Error occured while sending item to Redis: " + result));
        } else {
          resolve(null);
        }
      }
    ...
    client.on('timeout', function() {
      ...
      reject(new Error("Timeout occured when storing data in Redis"));
    });

    client.on('error', function(err) {
      console.error("Error occured when storing data in Redis:", err);
      reject(new Error("Error occured when storing data in Redis: " + err));
    });
  });
}
