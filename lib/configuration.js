...
const getEnvString = (name, defaultValue) => {
	if(process.env.hasOwnProperty(name)) {
		return process.env[name];
	}
	return defaultValue;
};
module.exports.getEnvString = getEnvString;
...
module.exports.getEnvNumber = getEnvNumber;
...
module.exports.getEnvBool = getEnvBool;

// Default values
const config = {
	configurationTable: getEnvString('CONFIGURATION_TABLE_NAME', getEnvString('AWS_LAMBDA_FUNCTION_NAME', 'fanout') + 'Targets'), // DynamoDB Table holding the list of targets for this function
	configRefreshDelay: getEnvNumber('CONFIGURATION_REFRESH_DELAY', 60000),                                                       // Reload configuration once very 5 minutes
	debug             : getEnvBool('DEBUG_MODE', false)                                                                           // Activate debug messages
};
...
// Runtime variables
const currentRegion = getEnvString('AWS_REGION', 'us-east-1');     // Current region from Lambda environment variables (where the function runs)
let   dynamo        = new AWS.DynamoDB({ region: currentRegion }); // DynamoDB service for loading the configuration, in the current region
	...
	return new Promise((resolve, reject) => {
		...
		dynamo.query(params).promise().then((data) => {
			...
			if(data.ExclusiveStartKey) {
				...
				LoadData(source, serviceDefinitions, targets, data.ExclusiveStartKey).then(resolve).catch(reject);
			} else {
