...
module.exports.xmodem = (buf) => {
	if((typeof buf) == "string") {
		buf = new Buffer(buf, "utf-8");
	}

	if(! Buffer.isBuffer(buf)) {
		throw new Error("xmodem / crc16 only supports buffers and strings");
	}

	let crc = 0;
	for(const value of buf) {
		for(let i = 0; i < 8; ++i) {
			const bit = ((value >> (7-i)) & 1) == 1;
			const c15 = (crc & 0x8000) !== 0;
			crc = (crc << 1) & 0xffff;
			if((c15 && (!bit)) || (bit && (!c15))) {
				crc = crc ^ 0x1021;
			}
